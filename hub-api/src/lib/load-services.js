...
module.exports = (app) => {
    ...
    const content = readdirSync(servicesPath).filter(f => !f.startsWith("_") && f.endsWith(".js"));
    ...
    for (let i = 0; i < content.length; i++) {
        ...
        const logger = require("./logger").createLogger({ service: `${name}-${version}` });

        require(fullPath)(app, publicRouter, privateRouter, logger);
        app.use(route, publicRouter);
        
        if (config.PRIVATE_ROUTES)
