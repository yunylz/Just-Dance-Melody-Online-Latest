 ...
 */
export function createLogger({ service, logToFile = false, logFilePath = "./log.txt", }) {
    if (!service) {
    ...
    const level = global.config?.LOG_LEVEL ||
        process.env.LOG_LEVEL ||
        (utils.isDev() ? "debug" : "info");
    const jsonFormat = winston.format.combine(winston.format.timestamp({ format: "YYYY-MM-DD_HH:mm:ss" }), winston.format.printf((info) => {
    ...
    if (lokiHost) {
        transports.push(new LokiTransport({
            ...
            format: winston.format.combine(winston.format.timestamp({ format: "YYYY-MM-DD_HH:mm:ss" }), winston.format.json()),
            json: true,
            batching: true,
            interval: 5,
