				...
				{#each Array.from({length: totalPages}, (_, i) => i + 1) as page}
					{#if page === 1 || page === totalPages || (page >= currentPage - 2 && page <= currentPage + 2)}
						<button
							...
							class="w-10 h-10 rounded-xl font-medium transition-all duration-300 {currentPage === page ? 'bg-gradient-to-r from-pink-500 to-red-500 text-white' : 'bg-gray-700/50 hover:bg-gray-600/50 border border-gray-600/50 text-gray-300 hover:text-white'}"
						>
							{page}
						</button>
