<script>
	...
	export let show;
	export let platform;
	export let platformTitle;
	export let platformIcon;
	export let message;
	...
	async function unlink() {
		isProcessing = true;
		dispatch('confirm', { platform });
		// Parent handles the logic and eventually we close or show error.
		// We simulate the processing state to match ConsoleUnlinkModal's UX.
...
<div
	...
	role="dialog"
	aria-modal="true"
	aria-label="Unlink social account"
	on:click|self={close}
	...
	<div class="cum-modal">
		...
		<div class="cum-header">
			<div class="cum-console-icon">
				<Icon icon={platformIcon} width="20" height="20" />
			</div>
			<div class="cum-header-text">
				<span class="cum-title">
					Unlink {platformTitle} Account
				</span>
		...
		<div class="cum-body">
			<p class="cum-warning">
				Are you sure you want to unlink your <strong>{platformTitle}</strong> account? You can re-link it later.
			</p>
